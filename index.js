...
let canSendRotationR = false, canSendRotationL = false;
...
function MarkerRotation() {
  ...
  if (rightHand) {
    ...
    if (markerRotation <= startingPointR) {
      canSendRotationR = true;
      if (canRotate) {
    ...
    if (canSendRotationR) {
      ...
      if (markerRotation > startingPointR && markerRotation <= firstZoneR -deadzone/200) {
        if (pastRotation != 1) {
      ...
      // still need to make "canSendRotationR" false after an action has been made in game
      if (markerRotation >= resetR) {
        canSendRotationR = false;
  ...
  // is the person using the palm marker with their left-hand?
    ...
    if (markerRotation <= startingPointL) {
      canSendRotationL = true;
      if (canRotate) {
        ipcRenderer.send('K_KEY_DOWN');
        ipcRenderer.send('K_KEY_UP');
        canRotate = false;
    ...
    if (canSendRotationL) {
      ...
      // is the marker then rotated to the first zone?
      if (markerRotation > startingPointL && markerRotation <= firstZoneL -deadzone/200) {
        if (pastRotation != 1) {
          ipcRenderer.send('L_KEY_DOWN');
          pastRotation = 1;
          ipcRenderer.send('L_KEY_UP');
      ...
      // still need to make "canSendRotationL" false after an action has been made in game
      if (markerRotation >= resetL) {
        canSendRotationL = false;
